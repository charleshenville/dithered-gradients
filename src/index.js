...
const TOTAL = Math.pow(WIDTH, 2) * HEIGHT
  ...
  async initScene() {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        numOctaves: params.octacves,
        renderMode: params.renderMd,
        randomFactor: params.randomFactor,
        colorVariance: params.colorVariance,
        pointSize: params.basePointScale
      },
    ...
    gui.add(params.octacves, "value", 1, 5, 1).name("numOctaves")
    gui.add(params.basePointScale, "value", 1.0, 3.5, 0.01).name("pointSize")
    ...
    gui.add(params, "renderMode", ["points", "canvas"]).name("renderMode").onChange((val) => {
      if (val == "points") {
        params.renderMd.value = 1
      } else if (val == "canvas") {
        params.renderMd.value = 2
      }
    })
    gui.add(params.noiseRange, "value", 0.1, 5, 0.1).name("noiseRange")
